...
def catch_all(path):
    ...
    try:
        ...
        if path == 'api/scan-file':
            # Get file info from FormData if available
            file_info = None
            if request.files and 'file' in request.files:
                file = request.files['file']
                file_info = {
                    'name': file.filename,
                    'size': 0  # We don't have actual size in serverless
                }
        ...
        # Even on error, return a proper formatted response instead of an exception
        error_response = {
            'status': 'error',
